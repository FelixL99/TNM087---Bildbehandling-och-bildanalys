...
Dkn = sqrt((X - floor(M/2) - uk).^2 + (Y - floor(N/2) - vk).^2);
Dkp = sqrt((X - floor(M/2) + uk).^2 + (Y - floor(N/2) + vk).^2);


H = (1./(1 + (D0./Dkn).^2)).*(1./(1+ (D0./Dkp).^2)); % The filter transfer function of the Notch bandreject filter
...
% 
% D0 = 12 tar bort allt noice helt i bilden.
%
% D0 = 10 tar bort det allra mesta noice, men om man zoomar in kan man
% fortfarande se vissa linjer i bakgrunden.
