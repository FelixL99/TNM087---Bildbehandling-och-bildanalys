...
n = 2 * round(n_formula/2) + 1; % the smallest odd size of the filter
...
im_filt = imfilter(im, fbox, 'replicate'); % the filtered image
...
% Your answer here: 
% Det minsta kernel filteret som vi kan använda för att ta bort objektet är
% 7x7. Om ett litet objekt är nära ett annat kommer kernel filtret
% behandla värdena i båda objekten. Vilket betyder att medelvärdet för kernel filtret
% kommer att vara större än Q/4 och kommer se objektet som större än det
% faktiskt är och inte filtrera bort det.
%